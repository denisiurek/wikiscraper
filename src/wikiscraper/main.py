...
def setup_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="WikiScraper: A tool to scrape and analyze wiki contents, "
                    "specifically Stardew Valley Wiki."
    )
    ...
    parser.add_argument(
        "--summary",
        type=str,
        help="Fetch the summary (first paragraph) of a given subpage "
             "(e.g., --summary 'Golden Walnut').",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Compare word frequency against a language standard. "
             "Requires --count-words or --auto-count-words to be run first.",
    )
...
def get_scraper_tool(config) -> WikiScraper:
    if config.mode == "stardew_normal":
        return StardewScraper(config)
    if config.mode == "stardew_file":
